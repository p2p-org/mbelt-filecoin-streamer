## Requirements
* Docker v1.11+
* Docker Compose
* Docker memory is allocated minimally at 8 GB

## Before start 
```shell script
./start-docker.sh
```

## What does the script do?
The script will start all necessary infrastructure: KSQLDB, Kafka, ZooKeeper, PostgreSQL containers, create all demo streams and connectors for DB (It'll take about 5 minutes).

## How to configure and start streamer
You can start mbelt-filecoin-streamer via docker-compose or via go build/run/install.

To start via docker compose, you need to modify env variables in docker-compose.yml and execution flags in Dockerfile (if needed) and then just run
```shell script
docker-compose up filecoin-streamer-worker
```

Or set necessary environment variables (see table below), go build or go install main.go in root of this repository and run it with or without execution flags (flags table below as well)   

Examples:
```shell script
go install github.com/p2p-org/mbelt-filecoin-streamer
mbelt-filecoin-streamer --sync-force --sub-head-updates=false
```
or

```shell script
go build -o mbelt-filecoin-streamer github.com/p2p-org/mbelt-filecoin-streamer
mbelt-filecoin-streamer --sync-from=1000
```

### How to start consistency watcher 

Along with streamer, which extract data, the watcher checks all 
blocks, tipsets and messages and their consistency.

To start via docker compose, you need to modify env variables in docker-compose.yml (mbelt-watchdog) and execution flags in Dockerfile (if needed) and then just run
```shell script
docker-compose up filecoin-watchdog
```

#### Environment variables

| Variable | Description | Example |
| ---- | ---- | ---- |
| `MBELT_FILECOIN_STREAMER_API_URL` | URL to filecoin node http API | `http://127.0.0.1:1234/rpc/v0` |
| `MBELT_FILECOIN_STREAMER_API_WS_URL` | URL to filecoin node websocket API | `ws://127.0.0.1:1234/rpc/v0` |
| `MBELT_FILECOIN_STREAMER_API_TOKEN` | Token for filecoin node API | |
| `MBELT_FILECOIN_STREAMER_KAFKA` | URL to Kafka | `http://127.0.0.1:9092` |
| `MBELT_FILECOIN_STREAMER_PG_URL` | URL to PostgreSQL DB with credentials needed to connect | `postgres://127.0.0.1:5432/raw?user=sink&password=123S&sslmode=disable` |

#### Execution flags

| Flag | Description | Default |
| ---- | ---- | ---- |
| `-s`, `--sync` | Turn on sync starting from last block in DB | `true` |
| `-f`, `--sync-force` | Turn on sync starting from genesis block | `false` |
| `-u`, `--sub-head-updates` | Turn on subscription on head updates | `true` |
| `-F`, `--sync-from` | Height to start sync from. Dont provide or provide negative number to sync from max height in DB | -1 |
| `-o`, `--sync-from-db-offset` | Specify offset from max height in DB to start sync from (maxHeightInDb - offset) | 100 |

## What is happening inside ksqlDB?
Data flow:
![flow](images/flow.png "Data flow")

Data that is being requested by mbelt-filecoin-streamer from filecoin node is being pushed to Kafka topics **tipsets_stream**, **blocks_stream**, **messages_stream**, **tipsets_to_revert_stream**, **actor_states_stream**.
Then this data is being pushed to corresponding ksqlDB streams (**TIPSETS_STREAM**, **BLOCKS_STREAM**, **MESSAGES_STREAM**, **TIPSETS_TO_REVERT_STREAM**, **ACTOR_STATES_STREAM**)  and further to corresponding streams with **_AVRO** suffix where they are being converted to AVRO format.
Then data is being sinked to PostgreSQL tables **filecoin._tipsets**, **filecoin._blocks**, **filecoin._messages**, **filecoin._tipsets_to_revert**, **filecoin._actor_states** (**_** prefix means that tables are temporary), then triggers on those tables convert data into more suitable for storing format and push them to corresponding tables with no **_** prefix in their name. Triggers also delete data from temporary tables after insertion to permanent ones.
Triggers on table **filecoin._tipsets_to_revert** on insertion delete corresponding tipsets from **filecoin.tipsets** table.   

## How does data sink to a DB?

KSQLDB has a special JDBCSinkConnector.

It collects data from some Kafka topic and inserts the data to a DB table.

## Message format examples

* **TIPSETS_STREAM** AND **TIPSETS_STREAM_AVRO** topics:

```json
[
  {
    "topic": "tipsets_stream",
    "partition": 0,
    "offset": 4761,
    "timestamp": 1603203660504,
    "timestampType": "CREATE_TIME",
    "headers": [],
    "key": "4538",
    "value": {
      "blocks": "{\"bafy2bzaceder2qjrmooe3ltgtcq5fjz3bxpkxaxf4twarrqsadriaav7u3cnk\", \"bafy2bzacedztxn3bsxcnozwoepmxrmpygldluibrnujpayunsgdiau3ctkuxk\", \"bafy2bzacebv77vl7sq67okysi6y3l6gb46qubwnjlq6quocidzqsa4gxajyvw\"}",
      "height": 4538,
      "min_timestamp": 1600299720,
      "parent_state": "bafy2bzacebqpq4ifxl42dwivawbz3mun6n2mdky67obvgsa4hbvnx5xvfwcki",
      "parent_weight": "71562251",
      "parents": "{\"bafy2bzacebrynbvd23piufeodv3rbvynij6gqzll3xp4i72jma33rwbqi7p42\", \"bafy2bzaced3jkpa2u35gma24qlxmkod2v3sqzavh5egmyyktocqfaw3qduzes\"}"
    }
  }
]
```

* **BLOCKS_STREAM** and **BLOCKS_STREAM_AVRO** topics:

 ```json
 [
   {
     "topic": "BLOCKS_STREAM_AVRO",
     "partition": 0,
     "offset": 6160,
     "timestamp": 1603203353315,
     "timestampType": "CREATE_TIME",
     "headers": [],
     "key": "bafy2bzacechfkocftlsezx6ge2v4r5bbr636d6lwab53d5gefq7ngapz4gm5q",
     "value": {
       "cid": {
         "string": "bafy2bzacechfkocftlsezx6ge2v4r5bbr636d6lwab53d5gefq7ngapz4gm5q"
       },
       "height": {
         "long": 2554
       },
       "parents": {
         "string": "{\"base_fee\":\"100\",\"cids\":[\"bafy2bzaceaqkc2gneaxji7yd4t734qfbpmyejizue56atffxoxcmzqps7k2ra\",\"bafy2bzacea7nrk3ni2di72jewzssjxsbld446oxg7rccpdvd5ri6cpoknjq2s\",\"bafy2bzacecxa2jzlsnvly5cbqoeuhohiiri7x2ciqocsjl63bwvuk6hl2pqlg\"],\"message_receipts\":\"bafy2bzacedswlcz5ddgqnyo3sak3jmhmkxashisnlpq6ujgyhe4mlobzpnhs6\",\"state_root\":{\"/\":\"bafy2bzaceat3bs6f67jxbwwskcjadaewjtpunjtk7vih4jj2yfb4wqlnkk6va\"},\"weight\":\"40483080\"}"
       },
       "win_count": {
         "int": 3
       },
       "miner": {
         "string": "t01001"
       },
       "messages_cid": {
         "string": "bafy2bzacecmda75ovposbdateg7eyhwij65zklgyijgcjwynlklmqazpwlhba"
       },
       "validated": {
         "boolean": false
       },
       "blocksig": {
         "string": "{\"data\":\"pRLPF6kCv2alh42awVOEv+wWLBZ8Ti9tYxJODvBtz7evve8ddQADkB/baKEJSvUlC7ofJznbAr2xKuL3WwtqEE7dWQO8EUWQ+ISekjSPK72BnhFXwA04htFe1Ws57o3J\",\"type\":2}"
       },
       "bls_aggregate": {
         "string": "{\"data\":\"wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\",\"type\":2}"
       },
       "block": {
         "string": "{\"Miner\":\"t01001\",\"Ticket\":{\"VRFProof\":\"sgXXT0dYKKQRJ0b3Unowb8mv4J/vt/tZqg7f9Z3IGlcTWPFVNuvAzMbHjccgxojwBwyJfjARMhv06JCWvQsPejWDiI2ZZNz92ui7gnXsZ/LsCBZDXCkq0eYRgHNWTf/3\"},\"ElectionProof\":{\"WinCount\":3,\"VRFProof\":\"hae3udpodlpSMw3j2v7dSciYhkvqAJ/C1mpHYBhIS9Y3zeARoO83mpb+DPxO61MlE9ghcere/cCZ40IgvGJLTmg9KNEVmtFX7m8cbwiBViy3mornCspu5gkrEAP9C/Mh\"},\"BeaconEntries\":[{\"Round\":160304,\"Data\":\"iaVfHeMp7d5CO8BGWfsbLWUXIfinSjVx2Hu+Ff7YjBfKcLlx3/NRsSz/HocmMxtVBevyAOLfx9IFqCAUVb71Veh6fRYdLI3QJABpYE3oH8oUPdS3Lig9OMmxIwahRj7i\"}],\"WinPoStProof\":[{\"PoStProof\":2,\"ProofBytes\":\"\"}],\"Parents\":[{\"/\":\"bafy2bzaceaqkc2gneaxji7yd4t734qfbpmyejizue56atffxoxcmzqps7k2ra\"},{\"/\":\"bafy2bzacea7nrk3ni2di72jewzssjxsbld446oxg7rccpdvd5ri6cpoknjq2s\"},{\"/\":\"bafy2bzacecxa2jzlsnvly5cbqoeuhohiiri7x2ciqocsjl63bwvuk6hl2pqlg\"}],\"ParentWeight\":\"40483080\",\"Height\":2554,\"ParentStateRoot\":{\"/\":\"bafy2bzaceat3bs6f67jxbwwskcjadaewjtpunjtk7vih4jj2yfb4wqlnkk6va\"},\"ParentMessageReceipts\":{\"/\":\"bafy2bzacedswlcz5ddgqnyo3sak3jmhmkxashisnlpq6ujgyhe4mlobzpnhs6\"},\"Messages\":{\"/\":\"bafy2bzacecmda75ovposbdateg7eyhwij65zklgyijgcjwynlklmqazpwlhba\"},\"BLSAggregate\":{\"Type\":2,\"Data\":\"wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\"},\"Timestamp\":1600240200,\"BlockSig\":{\"Type\":2,\"Data\":\"pRLPF6kCv2alh42awVOEv+wWLBZ8Ti9tYxJODvBtz7evve8ddQADkB/baKEJSvUlC7ofJznbAr2xKuL3WwtqEE7dWQO8EUWQ+ISekjSPK72BnhFXwA04htFe1Ws57o3J\"},\"ForkSignaling\":0,\"ParentBaseFee\":\"100\"}"
       },
       "block_time": {
         "long": 1600240200
       }
     }
   }
 ]
```

* **MESSAGES_STREAM** AND **MESSAGES_STREAM_AVRO** topics:

```json
[
  {
    "topic": "MESSAGES_STREAM_AVRO",
    "partition": 0,
    "offset": 219,
    "timestamp": 1603203541682,
    "timestampType": "CREATE_TIME",
    "headers": [],
    "key": "bafy2bzacebmai53zksy2jdoyyoofg7wxzk54bfuohwkqao6e3c2fq6gq2s6h6",
    "value": {
      "cid": {
        "string": "bafy2bzacebmai53zksy2jdoyyoofg7wxzk54bfuohwkqao6e3c2fq6gq2s6h6"
      },
      "block_cid": {
        "string": "bafy2bzaceakya2lxdl3cr3vy27vzsxzkjvpsi5gdnfemq4fz5fbzeb5fkpv2u"
      },
      "method": {
        "int": 5
      },
      "from": {
        "string": "t3w6fpozxps63fgfld6fo5csrytyq33n6lppredlcmcyt6sf5mbyvkpk6n2w7fhgiyp665nceayggqnx4wjroq"
      },
      "to": {
        "string": "t01000"
      },
      "value": {
        "long": 0
      },
      "gas": {
        "string": "{\"fee_cap\":\"150493\",\"limit\":232917948,\"premium\":\"150169\"}"
      },
      "params": {
        "string": ""
      },
      "data": {
        "string": "{\"Version\":0,\"To\":\"t01000\",\"From\":\"t3w6fpozxps63fgfld6fo5csrytyq33n6lppredlcmcyt6sf5mbyvkpk6n2w7fhgiyp665nceayggqnx4wjroq\",\"Nonce\":52,\"Value\":\"0\",\"GasLimit\":232917948,\"GasFeeCap\":\"150493\",\"GasPremium\":\"150169\",\"Method\":5,\"Params\":\"\"}"
      },
      "block_time": {
        "long": 1600275330
      }
    }
  }
]
```

* **ACTOR_STATES_STREAM** AND **ACTOR_STATES_STREAM_AVRO** topics:

```json
[
  {
    "topic": "actor_states_stream",
    "partition": 0,
    "offset": 35,
    "timestamp": 1603964771584,
    "timestampType": "CREATE_TIME",
    "headers": [],
    "key": "t01000_{bafy2bzaceckm3rdrzmf5og3w4bbihnwf7xe5bvsibh5yv2edxztvroojahvqw,bafy2bzacecmabxuahm3evx37los7lt4mnaxg7i4ik6jplfxv2nnwfebb2a7f4,bafy2bzacedpqwacyhdwoopxpjafeadar2r6hplzegijxyoqqfsxjsmxheogki}",
    "value": {
      "actor_code": "bafkqaetgnfwc6mrpon2g64tbm5sw22lomvza",
      "actor_head": "bafy2bzacebwptgbq2nlmyn4ucsjz2lum4qo7nykqr34mcbe6ixuw53ozpdot6",
      "actor_state_key": "t01000_{bafy2bzaceckm3rdrzmf5og3w4bbihnwf7xe5bvsibh5yv2edxztvroojahvqw,bafy2bzacecmabxuahm3evx37los7lt4mnaxg7i4ik6jplfxv2nnwfebb2a7f4,bafy2bzacedpqwacyhdwoopxpjafeadar2r6hplzegijxyoqqfsxjsmxheogki}",
      "addr": "t01000",
      "balance": "4576662981025374931151",
      "height": 375,
      "is_account_actor": false,
      "nonce": 0,
      "parent_ts_key": "{bafy2bzacea7vaiwd3hzs3ejgiy55dozaacrukk6gcil6v2vy5766arxlnwovg,bafy2bzacebw6mmemqngcmhklfn5zfqxwbolyf4gxzjo5r7h5wv4dik7fh3w3g,bafy2bzacedyxbbjr4dhge2hyqjgz326eowhudnviqfrxq5f3dxcelryg6hdpg}",
      "state": "{\"AllocatedSectors\":{\"/\":\"bafy2bzacebyt5fitbtfj6qk2f5qeorfl7kfiriab547olwh7krzspjq2kv34c\"},\"CurrentDeadline\":0,\"Deadlines\":{\"/\":\"bafy2bzacedwohbh2mede6itj74m7onl4nv4ejrs6dmlnm2rxuc42inhmi3jv6\"},\"EarlyTerminations\":[0],\"FeeDebt\":\"0\",\"Info\":{\"/\":\"bafy2bzacea52isokwuop2rcqbie5uvltkpmdjt7mg5povvsmp3avih2az4xli\"},\"InitialPledge\":\"159999997489079910400\",\"LockedFunds\":\"4141528229167716516047\",\"PreCommitDeposits\":\"0\",\"PreCommittedSectors\":{\"/\":\"bafy2bzaceamp42wmmgr2g2ymg46euououzfyck7szknvfacqscohrvaikwfay\"},\"PreCommittedSectorsExpiry\":{\"/\":\"bafy2bzaceawdouk3zqz2pm25qbsv5bzk2uxsbhsqm7gvsbmryk4cwsjqsbkta\"},\"ProvingPeriodStart\":658,\"Sectors\":{\"/\":\"bafy2bzacectjy4uapslgwlet5ue4en6nqk6zgvzg23f4jeip5szn6zhnti6he\"},\"VestingFunds\":{\"/\":\"bafy2bzacedjryafr2b3pqko3ipm7drmvwkimwug7oxnvimqd6goc6q2w5gp46\"}}",
      "state_root": "bafy2bzacebzf6of2fdkfbibw3mhxyucw7r4le3yahlwacgfx5l3sxznq547z4",
      "ts_key": "{bafy2bzaceckm3rdrzmf5og3w4bbihnwf7xe5bvsibh5yv2edxztvroojahvqw,bafy2bzacecmabxuahm3evx37los7lt4mnaxg7i4ik6jplfxv2nnwfebb2a7f4,bafy2bzacedpqwacyhdwoopxpjafeadar2r6hplzegijxyoqqfsxjsmxheogki}"
    }
  }
]
```

Input and enrichment data are intentionally non-optimal to test and show how different edge cases are handled

## End-to-end latency of streams(topics)
* **TIPSETS_STREAM** (intermediate stream) - 3ms
* **TIPSETS_STREAM_AVRO** (raw tipsets data sinking to Postgres) - 63ms
* **BLOCKS_STREAM** (intermediate stream) - 3ms
* **BLOCKS_STREAM_AVRO** (raw block's data sinking to Postgres) - 63ms
* **MESSAGES_STREAM** (intermediate stream to process data) - 3ms
* **MESSAGES_STREAM_AVRO** (parsed extrinsic's data sinking to Postgres) - 63ms
* **ACTOR_STATES_STREAM** (intermediate stream) - 3ms
* **ACTOR_STATES_STREAM_AVRO** (raw tipsets data sinking to Postgres) - 63ms

## How to see PoC?

Navigate to the Control Center web interface at http://localhost:9021/ and select your cluster.
![cluster](images/cluster.png "Cluster")

You'll see an overview of the cluster and some tabs:
![overview](images/overview.png "Overview")

* **Topics** tab contains all topics created by the script and streams. Click by a topic and you see a full statistic about it.
![topic](images/topic.png "Topic")

* **Connect** tab contains an info about all connectors in the system.
![connect](images/connect.png "Connect")

* **ksqlDB** tab provides an interface for working with queries, streams and tables
![ksqldb](images/ksqlDB.png "ksqlDB")
    * Sub-tab **Running queries** provides information about running queries in the system.
    ![queries](images/queries.png "Queries")

## GraphQL API

Before you run API you will need to apply migrations. Run migrator via docker compose:

```shell script
docker-compose up -d migrator
```

or set PG_URL (URL to PostgreSQL DB) environment variable and run it without docker. Example:

```shell script
go run post-graphile-migrations migrator.go
```

Then you need to start PostGraphile. Run it via docker compose:

```shell script
docker-compose up -d migrator
```

or

```shell script
postgraphile --subscriptions --dynamic-json --no-setof-functions-contain-nulls --no-ignore-rbac --show-error-stack=json --graphiql "/"  --export-schema-graphql schema.graphql --enable-query-batching --legacy-relations omit --schema graphql --connection {POSTGRES_URL} --watch  
```

You can use PostGraphile UI via http://localhost:5000/

To do queries, do POST requests to
```bash
POST http://localhost:5000/graphql
```

Example:
```bash
curl --request POST 'http://localhost:5000/graphql' \
--header 'Content-Type: application/json' \
--data '{
	"query": "query { allBlocks(first: 1) { edges { node { cid height parents winCount msgCount miner validated blocksig blsAggregate block blockTime } } } }",
	"variables": null
}'
```

Response:
```json
{
  "data": {
    "allBlocks": {
      "edges": [
        {
          "node": {
            "cid": "bafy2bzaceduqbenipcvmlhjcr5pxlpjqdpqudp3a254blvbizq75vevpzeaxm",
            "height": "7637",
            "parents": {
              "cids": [
                "bafy2bzacect5rifqtregqymftvguus32xxosdkf2hhcld5pzypmlhz3gobijq",
                "bafy2bzaced3jp7gze45ljknutlz4xsv5sxjkmapbrtj4nzndw2tsoopmguark"
              ],
              "weight": "120877935",
              "base_fee": "100",
              "state_root": {
                "/": "bafy2bzaceajgj44nfidlxcvdueu3ra3wmf7p2fmpfufew4leyyx2hrgs2z5mg"
              },
              "message_receipts": "bafy2bzacedswlcz5ddgqnyo3sak3jmhmkxashisnlpq6ujgyhe4mlobzpnhs6"
            },
            "winCount": 3,
            "msgCount": "0",
            "miner": "t01001",
            "validated": false,
            "blocksig": {
              "data": "gf037OGMsZET3fRIT3sirIETMW3BZYYLKUyz15+ImI4AFq5OcQfoQB6YcUAHpGGMFpS84/ZbcfbPhzWRmD7gglRm084picmI9rNLGJPjjP5H01PCJoPUYlaEepY2eX2Q",
              "type": 2
            },
            "blsAggregate": {
              "data": "wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
              "type": 2
            },
            "block": {
              "Miner": "t01001",
              "Height": 7637,
              "Ticket": {
                "VRFProof": "uPZCTamFLdMmjEL1s5k06lkOyiWdfuCr79NOciM6G7cH1t9JZHkn5QLdjtazvHJsBnogI4VbGxhVBb8m5p3PrCJAuMUJeDASwl6LBTXd9Ej6E94ZUMKhciPTy3ruAgtb"
              },
              "Parents": [
                {
                  "/": "bafy2bzacect5rifqtregqymftvguus32xxosdkf2hhcld5pzypmlhz3gobijq"
                },
                {
                  "/": "bafy2bzaced3jp7gze45ljknutlz4xsv5sxjkmapbrtj4nzndw2tsoopmguark"
                }
              ],
              "BlockSig": {
                "Data": "gf037OGMsZET3fRIT3sirIETMW3BZYYLKUyz15+ImI4AFq5OcQfoQB6YcUAHpGGMFpS84/ZbcfbPhzWRmD7gglRm084picmI9rNLGJPjjP5H01PCJoPUYlaEepY2eX2Q",
                "Type": 2
              },
              "Messages": {
                "/": "bafy2bzacecmda75ovposbdateg7eyhwij65zklgyijgcjwynlklmqazpwlhba"
              },
              "Timestamp": 1600392690,
              "BLSAggregate": {
                "Data": "wAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
                "Type": 2
              },
              "ParentWeight": "120877935",
              "WinPoStProof": [
                {
                  "PoStProof": 2,
                  "ProofBytes": ""
                }
              ],
              "BeaconEntries": [
                {
                  "Data": "ifn15S8pW1slIkNAP4mRyidzvfqwTuRAmFjO8EfPBMTqN2i0GlpzZ33ePkcyo85JD3LkzQN5mhP5uuWaQ7/fE6pbmVZmSRY0dgkXWCAo+2sMvPvvaZfTPO74vavchGVW",
                  "Round": 165387
                }
              ],
              "ElectionProof": {
                "VRFProof": "hS3JYA9rHB6j5irvXCdHV50KZTDsXNhggIZWr7UUA1zIiGcHtzRExnKYYfo9H028B1yXj4owlAz4WMJDqRcZAt9+tnQ2Z2ZGVMhJEudOUxrLQNlGGz/MM7xxD/uijNut",
                "WinCount": 3
              },
              "ForkSignaling": 0,
              "ParentBaseFee": "100",
              "ParentStateRoot": {
                "/": "bafy2bzaceajgj44nfidlxcvdueu3ra3wmf7p2fmpfufew4leyyx2hrgs2z5mg"
              },
              "ParentMessageReceipts": {
                "/": "bafy2bzacedswlcz5ddgqnyo3sak3jmhmkxashisnlpq6ujgyhe4mlobzpnhs6"
              }
            },
            "blockTime": "2020-09-18T01:31:30"
          }
        }
      ]
    }
  }
}
```

## The list of [extra queries](QUERIES.md)