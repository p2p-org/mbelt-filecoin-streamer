...
setupResp = requests.post(url=redashUrl + "/setup", data=setupPayload, headers=setupHeaders, allow_redirects=False)
print('User created')
...
usersResp = requests.get(url=redashUrl + "/api/users/1", cookies=setupResp.cookies)
...
for fileName in glob.iglob(queriesDir + '*.sql', recursive=True):
    ...
    queryResp = redash.create_query(ds_id=datasourceId, name=queryName, qry=query, desc=queryDescription)
    ...
    if len(visualization) > 3:
        ...
        print('Created visualisation for', queryName, 'query. Visualization id:', visId)

        redash.generate_query_results(ds_id=datasourceId, qry=query, qry_id=queryId)
